...
async function parse (path, $refs, options) {
  ...
  try {
    ...
    else if (options.allowErrors) {
      $ref.pathType = ex.plugin.name;
      file.data = null;
      $ref.value = null;
      $ref.errors = ex.plugin.error.errors;
      return $ref.value;
    }
    else {
      throw ex.error;
...
function parseFile (file, options, $refs) {
  return new Promise(((resolve, reject) => {
    ...
    function onParsed (parser) {
      if ((options.allowErrors || !parser.plugin.allowEmpty) && isEmpty(parser.result)) {
