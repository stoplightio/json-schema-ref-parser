...
module.exports = function (root) {
  ...
  return {
    ...
    suggestNameForUrl (schema, url) {
      if (!computed[url]) {
        try {
          const { query, href } = parse(url, true);
          if (typeof query.srn === "string") {
            let ext = extname(query.srn);
            computed[url] = this.suggestNameForFilePath(schema, basename(query.srn, ext) + (query.mid ? `.mid${query.mid}${ext || ".mid"}` : ext));
          }
          else {
