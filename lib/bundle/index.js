...
function inventory$Ref ($refParent, $refKey, path, pathFromRoot, indirections, inventory, $refs, options, customRoots) {
  ...
  let inlineable = options.bundle.shouldInline === null || options.bundle.shouldInline(pathFromRoot);
  ...
  for (let entry of inventory) {
    ...
    if (!entry.inlineable && customRoots[entry.file] && customRoots[entry.file][entry.hash]) {
      if (entry.hash === "#") {
        ...
        let value = get(subschema, "#/" + parsedHash.slice(0, parsedHash.length - 1).join("/"));
        set(schema, customRoots[entry.file][entry.hash], value[parsedHash[parsedHash.length - 1]]);
