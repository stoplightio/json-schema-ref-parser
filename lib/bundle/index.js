...
function bundle (parser, options) {
  ...
  let inventory = [];
  let roots = {
    custom: {}, // custom roots, computed by us
    normal: {}, // these are original paths to properties, scoped to a root
  };

  crawl(parser, "schema", parser.$refs._root$Ref.path + "#", "#", 0, inventory, parser.$refs, options, roots);

  // Remap all $ref pointers
  remap(parser.schema, inventory, options, roots);
}
 ...
 * @param {$RefParserOptions} options
 * @param {object} roots
 * @param {object} roots.custom
 * @param {object} roots.normal
 */
function crawl (parent, key, path, pathFromRoot, indirections, inventory, $refs, options, roots) {
  let obj = key === null ? parent : parent[key];

  if (obj && typeof obj === "object" && !ArrayBuffer.isView(obj)) {
    if ($Ref.isAllowed$Ref(obj)) {
      inventory$Ref(parent, key, path, pathFromRoot, indirections, inventory, $refs, options, roots);
    }
    else {
      ...
      for (let key of keys) {
        ...
        if ($Ref.isAllowed$Ref(value)) {
          inventory$Ref(obj, key, path, keyPathFromRoot, indirections, inventory, $refs, options, roots);
        }
        else {
          crawl(obj, key, keyPath, keyPathFromRoot, indirections, inventory, $refs, options, roots);
        }
      }
    }
  }
...
function findClosestRoot (roots, file, hash, pathFromRoot) {
  let fileRoots = roots.custom[file];
  let keys = Object.keys(fileRoots);
  if (keys.length > 0 && fileRoots["#"] === null) {
    return pathFromRoot;
  }

  keys = keys.filter(key => fileRoots[key] !== null);
  keys.sort((a, b) => b.length - a.length);
  let customRoot = keys.find(root => hash.startsWith(root));

  return fileRoots[customRoot] + hash.replace(customRoot, "");
}
 ...
 * @param {object} roots.normal
 */
function inventory$Ref ($refParent, $refKey, path, pathFromRoot, indirections, inventory, $refs, options, roots) {
  ...
  let inlineable = external && options.bundle.shouldInline(pathFromRoot);
  if (!inlineable && file !== $refs._root$Ref.path) {
    let customRoots = roots.custom;

    if (!customRoots[file]) {
      customRoots[file] = {};

      if (!roots.normal[file]) {
        customRoots[file]["#"] = options.bundle.generateKey($refs._root$Ref.value, file, null, pathFromRoot);
      }
    }
    ...
    pathFromRoot = findClosestRoot(roots, file, hash, pathFromRoot);
    ...
    roots.normal[file][hash] = pathFromRoot;
  }
  ...
  if (!existingEntry) {
    crawl(pointer.value, null, pointer.path, pathFromRoot, indirections + 1, inventory, $refs, options, roots);
  }
}
 ...
 * @param {object} roots.normal
 * */
function remap (schema, inventory, options, roots) {
  ...
  for (let entry of inventory) {
    ...
    if (!entry.inlineable && customRoots[entry.file] && entry.hash in customRoots[entry.file] && customRoots[entry.file]["#"] !== null) {
      ...
      // entry is not supposed to be moved anywhere, it's not placed under root
        ...
        let subschema = get(schema, customRoots[entry.file]["#"] || roots.normal[entry.file]["#"]);
